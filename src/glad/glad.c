/*
    ...
    OpenGL loader generated by glad 0.1.36 on Mon Mar 24 17:58:43 2025.
...
typedef void *(APIENTRYP PFNWGLGETPROCADDRESSPROC_PRIVATE)(char const *);
...
static int get_exts(void) {
  ...
  if (max_loaded_major < 3) {
    ...
    for (index = 0; index < num_exts_i; index++) {
      char const *gl_str_tmp = (char const *)glGetStringi(GL_EXTENSIONS, index);
...
static int has_ext(char const *ext) {
#ifdef _GLAD_IS_SOME_NEW_VERSION
  if (max_loaded_major < 3) {
#endif
...
static void find_coreGL(void) {
  ...
  char const *version;
  char const *prefixes[] = {"OpenGL ES-CM ", "OpenGL ES-CL ", "OpenGL ES ",
                            NULL};
  ...
  for (i = 0; prefixes[i]; i++) {
    size_t const length = strlen(prefixes[i]);
